...
function Withdrawals() {
  ...
  const [voidModal, setVoidModal] = useState<{ open: boolean; id: string; reason: string }>({
    open: false,
    id: '',
    reason: '',
  });
  ...
  const voidMutation = useMutation({
    mutationFn: ({ id, reason }: { id: string; reason: string }) =>
      withdrawalApi.void(id, { reason }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['withdrawals'] });
      queryClient.invalidateQueries({ queryKey: ['dashboard'] });
      addToast('success', 'Withdrawal voided');
      setVoidModal({ open: false, id: '', reason: '' });
    },
    onError: (err: any) => {
      addToast('error', err.message || 'Failed to void withdrawal');
    },
  });
                      ...
                      <td>
                        {!item.voidedAt ? (
                          <button
                            ...
                            onClick={() => setVoidModal({ open: true, id: item.id, reason: '' })}
                          >
                            Void
                          </button>
                        ) : (
        ...
        <div className="modal-overlay">
          <div className="modal">
            <div className="modal__header">
              <h3 className="modal__title">Void Withdrawal Entry</h3>
              <button className="btn btn-ghost btn-sm" onClick={() => setVoidModal({ open: false, id: '', reason: '' })}>✕</button>
            </div>
            ...
            <div className="modal__footer">
              <button className="btn btn-secondary" onClick={() => setVoidModal({ open: false, id: '', reason: '' })}>Cancel</button>
              <button
                className="btn btn-danger"
                disabled={!voidModal.reason || voidMutation.isPending}
                onClick={() => voidMutation.mutate({ id: voidModal.id, reason: voidModal.reason })}
              >
                Confirm Void
              </button>
            </div>
